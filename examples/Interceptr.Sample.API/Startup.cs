...
namespace Interceptr.Sample.API
{
    public class Startup
    {
        ...
        {
            services.AddScopedIntercepted<ISampleService, SampleService>(
                new Interceptr.ChronometerInterceptor(),
                new StartingFinishingInterceptor());

            services.AddOtherLayers();

            services.AddControllers();
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "Interceptr.Sample.API", Version = "v1" });
            });
        }
